...
export default function OrderList() {
    ...
    const renderOrderItem = (item, key) => {
        ...
        let totalPrice = 0
        item?.list_cart_item_id?.map(item => totalPrice += item?.item_total_price)

        return (
            <TouchableOpacity style={styles.orderItem} key={key} onPress={() => navigation.navigate("TrackingOrder", { orderDetail: item })}>
                <View style={styles.orderItemHeader}>
                    <Text style={styles.orderItemHeaderId} numberOfLines={1}>ID: {item._id}</Text>
                    ...
                    <View style={styles.orderItemInfoBody}>
                        <>
                            {item?.list_cart_item_id?.map((plant, index) => {
                                return (
                                    <View key={index}>
                                        <Text style={styles.plantName}>{plant?.plant_id?.name}</Text>
                                        <View style={{ ...styles.flexRow, justifyContent: "space-between" }}>
                                            <Text style={styles.plantPrice}>{formatPrice(plant?.plant_id?.price)} VNĐ</Text>
                        ...
                        <View style={styles.orderItemInfoBottom}>
                            <Text style={styles.totalPrice}>Total {formatPrice(totalPrice || 0)} VNĐ</Text>
                        </View>
                    </View>
                </View>
        ...
        <>
            <SafeAreaWrapper >
                ...
                <ScrollView style={styles.container}>
                    {!loading ? orderList.map((item, key) => renderOrderItem(item, key)) : <SpinnerLoading />}
                </ScrollView>
            </SafeAreaWrapper>
            <MenuModal
                visible={menuVisible}
